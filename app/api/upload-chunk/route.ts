...
const pendingPublicUploads = new Map<
  string,
  {
...
function cleanupStaleUploads() {
  const now = Date.now();
  for (const [id, upload] of pendingPublicUploads.entries()) {
    if (now - upload.createdAt > 15 * 60 * 1000) {
      pendingPublicUploads.delete(id);
    }
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    let pending = pendingPublicUploads.get(uploadId);
    if (!pending) {
      pending = {
        chunks: new Map(),
        totalChunks,
      ...
      pendingPublicUploads.set(uploadId, pending);
    }
    ...
    if (pending.chunks.size === totalChunks) {
      ...
      pendingPublicUploads.delete(uploadId);
    ...
    console.error("Public Chunk Upload Error:", err);
    return NextResponse.json(
      {
        success: false,
